...
namespace Microsoft.PowerToys.Settings.UI.ViewModels
{
    public class DashboardViewModel : Observable
    {
        ...
        public ObservableCollection<DashboardListItem> InactiveModules { get; set; } = new ObservableCollection<DashboardListItem>();
        ...
        public DashboardViewModel(ISettingsRepository<GeneralSettings> settingsRepository)
        {
            ...
            InactiveModules = new ObservableCollection<DashboardListItem>(_allModules.Where(x => !x.IsEnabled));
        ...
        public void ModuleEnabledChangedOnSettingsPage()
        {
            ...
            foreach (DashboardListItem item in _allModules)
            {
                ...
                if (item.IsEnabled)
                {
                    if (InactiveModules.Contains(item))
                    {
                        InactiveModules.Remove(item);
                        ActiveModules.Insert(0, item);
                    }
                }
                else
                {
                    if (ActiveModules.Contains(item))
                    {
                        ActiveModules.Remove(item);
                        InactiveModules.Insert(0, item);
                    }
                }
            }
        }
